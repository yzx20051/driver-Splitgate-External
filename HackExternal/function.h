...
void PrintPtr1(std::string text, uintptr_t ptr) {
...
FTransform GetBoneIndex(DWORD_PTR mesh, int index)
{
	...
	PrintPtr1("Bone Array ", bonearray);
...
Vector3 ProjectWorldToScreen(Vector3 WorldLocation)
{
	...
	//PrintPtr1("Camera Manager", CameraManager);
...
static bool WorldToScreen(uintptr_t PlayerController, Vector3 WorldLocation, Vector2* ScreenLocation)
{
	typedef bool(__fastcall* t_ProjectWorldLocationToScreen)(uintptr_t PlayerController, Vector3 WorldLocation, Vector2* ScreenLocation, bool bPlayerViewportRelative);
	t_ProjectWorldLocationToScreen sub_ProjectWorldLocationToScreen = (t_ProjectWorldLocationToScreen)((uintptr_t)GetModuleHandle(NULL) + 0x29D2740);

	return sub_ProjectWorldLocationToScreen((uintptr_t)PlayerController, WorldLocation, ScreenLocation, false);
}

Vector3 ProjectWorldToScreen1(Vector3 world_location)
{
	Vector3 Screenlocation = Vector3(0, 0, 0);
	...
	if (CFG.FOVChanger)
	{
		Screenlocation.x = screen_center_x + vtransformed.x * ((screen_center_x / tanf(CFG.fov * M_PI / 360))) / vtransformed.z;
		Screenlocation.y = screen_center_y - vtransformed.y * ((screen_center_x / tanf(CFG.fov * M_PI / 360))) / vtransformed.z;
	}
	else
	{
