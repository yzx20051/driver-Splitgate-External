...
auto CallHacks()->VOID
{
	while (true)
	{
		auto gamePlayerController = read<DWORD_PTR>(GameVars.local_player + GameOffset.offset_player_controller);
		auto gameMyPlayer = read<DWORD_PTR>(gamePlayerController + GameOffset.offset_apawn);
		auto gameMyGun = read<uint64_t>(gameMyPlayer + 0x800);
		auto gameMyGunRecoilConfig = read<DWORD_PTR>(gameMyGun + 0x344);
		auto gameMyGunCurrentAmmo = read<uint16_t>(gameMyGun + 0x302);


		if (CFG.b_NoRecoil)
		{
			write<float>(gameMyGun + 0x344 + 0x8, 0.0f);
			write<float>(gameMyGun + 0x344 + 0xc, 0.0f);
		...
		if (CFG.airstuck)
		{
			...
			{
				write<float>(GameVars.local_player_pawn + 0x98, 0.03f);
			}
		}
		...
		if (CFG.FOVChanger)
		{
			write<float>(CameraManager + 0x23C, CFG.fov);
		}
	...
	while (true)
	{
		...
		if (isAimbotActive)
		{
			...
			for (int index = 0; index < EntityList_Copy.size(); ++index)
			{
				...
				auto Health1 = read<float>(Entity.actor_pawn + GameOffset.offset_health);
				auto MaxHealth1 = read<float>(Entity.actor_pawn + GameOffset.offset_max_health); // idk how this works lmao
				auto procentage1 = Health1 * 100 / MaxHealth1;

				if (procentage1 == 0)
					continue;

				auto MyTeam = read<char>(GameVars.local_player_state + 0x338);
				auto Team = read<char>(Entity.actor_state + 0x338);

				if (MyTeam == Team)
					continue;

				auto head_pos = GetBoneWithRotation(Entity.actor_mesh, bones::head);
				auto targethead = ProjectWorldToScreen1(Vector3(head_pos.x, head_pos.y, head_pos.z + 15));
...
auto GameCache()->VOID
{
	while (true)
	{
		...
		for (int index = 0; index < GameVars.actor_count; ++index)
		{
			...
			if (afaf) //name == "PortalWarsBot_BP_C" || name == "PortalWarsCharacter_BP_C"
			{			
...
auto RenderVisual()->VOID
{
	...
	for (int index = 0; index < EntityList_Copy.size(); ++index)
	{
		...
		auto BottomBox = ProjectWorldToScreen1(bone_pos);
		auto TopBox = ProjectWorldToScreen1(Vector3(head_pos.x, head_pos.y, head_pos.z + 15));
		...
		auto ESP_Color = ImColor(255, 255, 255, 255); //GetVisibleColor(bVisible);

		auto actor_pawn1 = read<uintptr_t>(GameVars.actors + index * 0x8);
		auto actor_id1 = read<int>(actor_pawn1 + GameOffset.offset_actor_id);
		auto actor_mesh1 = read<uintptr_t>(actor_pawn1 + GameOffset.offset_actor_mesh);
		auto actor_state1 = read<uintptr_t>(actor_pawn1 + GameOffset.offset_player_state);
		auto name = GetNameFromFName(actor_id1);

		auto Health1 = read<float>(Entity.actor_pawn + GameOffset.offset_health);
		auto MaxHealth1 = read<float>(Entity.actor_pawn + GameOffset.offset_max_health); // idk how this works lmao
		auto procentage1 = Health1 * 100 / MaxHealth1;

		auto Bot = read<char>(Entity.actor_state + 0x22a);
		...
		if (CFG.b_Visual)
		{
			if (entity_distance < CFG.max_distance)
			{
				if (CFG.b_EspBox)
				{
					if (CFG.BoxType == 0)
					{
						DrawBox(TopBox.x - (CornerWidth / 2), TopBox.y, CornerWidth, CornerHeight, ImColor(30, 100, 0, 255));
					}
					else if (CFG.BoxType == 1)
					{
						DrawCorneredBox(TopBox.x - (CornerWidth / 2), TopBox.y, CornerWidth, CornerHeight, ImColor(30, 100, 0, 255), 1.5);
					}
					else if (CFG.BoxType == 2)
					{
						Draw3DBox(TopBox, BottomBox, ESP_Color);
					}
				}
				if (CFG.b_EspLine)
				{
				...
				if (CFG.b_EspDistance)
				{
					...
					DrawOutlinedText(Verdana, dist, ImVec2(TopBox.x, TopBox.y - 40), 14.0f, ImColor(255, 255, 255), true);
				}
				if (CFG.b_EspAmmo)
				{
					auto gameMyGun1 = read<uint64_t>(Entity.actor_pawn + 0x800);
					auto gameMyGunRecoilConfig1 = read<DWORD_PTR>(gameMyGun1 + 0x344);
					auto gameMyGunCurrentAmmo1 = read<uint16_t>(gameMyGun1 + 0x302);

					char ammo[64];
					sprintf_s(ammo, "Ammo: %.f", gameMyGunCurrentAmmo1);

					DrawOutlinedText(Verdana, ammo, ImVec2(BottomBox.x, BottomBox.y + 20), 14.0f, ImColor(255, 255, 255), true);
				}

				if (CFG.b_EspName)
				{
					auto gameEnemyGun = read<FString>(Entity.actor_pawn + 0x800);

					char dist[64];
					sprintf_s(dist, " [%.fm]", entity_distance);

					float score11 = read<float>(Entity.actor_state + 0x220);

					char score[64];
					sprintf_s(score, "[%.f] ", score11);

						auto PlayerName = read<FString>(Entity.actor_state + GameOffset.offset_player_name);
						DrawOutlinedText(Verdana, score + PlayerName.ToString() + dist, ImVec2(TopBox.x, TopBox.y - 20), 14.0f, ImColor(255, 255, 255), true);
				...
				if (CFG.b_EspHealth)
				{
					...
					char heal[64];
					sprintf_s(heal, "%.f HP", procentage);
					DrawOutlinedText(Verdana, heal, ImVec2(BottomBox.x, BottomBox.y + 10), 14.0f, ImColor(255, 255, 255), true);
				}
				if (CFG.b_EspSkeleton)
				{

						Vector3 vHeadOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::head));
						Vector3 vNeckOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::neck_01));
						Vector3 vSpine1Out = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::spine_01));
						Vector3 vSpine2Out = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::spine_02));
						Vector3 vSpine3Out = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::spine_03));
						Vector3 vPelvisOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::pelvis));
						Vector3 vRightThighOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::thigh_r));
						Vector3 vLeftThighOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::thigh_l));
						Vector3 vRightCalfOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::calf_r));
						Vector3 vLeftCalfOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::calf_l));
						Vector3 vRightFootOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::foot_r));
						Vector3 vLeftFootOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::foot_l));
						Vector3 vUpperArmRightOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::upperarm_r));
						Vector3 vUpperArmLeftOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::upperarm_l));
						Vector3 vLowerArmRightOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::lowerarm_r));
						Vector3 vLowerArmLeftOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::lowerarm_l));
						Vector3 vHandRightOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::hand_r));
						Vector3 vHandLeftOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, bones::hand_l));
						Vector3 vFootOutOut = ProjectWorldToScreen1(GetBoneWithRotation(Entity.actor_mesh, 0));

						DrawCircle(vHeadOut.x, vHeadOut.y, 5.f, ImColor(255, 0, 0, 255), 1000);

						DrawLine(ImVec2(vHeadOut.x, vHeadOut.y), ImVec2(vNeckOut.x, vNeckOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vNeckOut.x, vNeckOut.y), ImVec2(vSpine1Out.x, vSpine1Out.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vSpine1Out.x, vSpine1Out.y), ImVec2(vSpine2Out.x, vSpine2Out.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vSpine2Out.x, vSpine2Out.y), ImVec2(vSpine3Out.x, vSpine3Out.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vSpine1Out.x, vSpine1Out.y), ImVec2(vUpperArmRightOut.x, vUpperArmRightOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vSpine1Out.x, vSpine1Out.y), ImVec2(vUpperArmLeftOut.x, vUpperArmLeftOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vSpine1Out.x, vSpine1Out.y), ImVec2(vPelvisOut.x, vPelvisOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vPelvisOut.x, vPelvisOut.y), ImVec2(vRightThighOut.x, vRightThighOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vPelvisOut.x, vPelvisOut.y), ImVec2(vLeftThighOut.x, vLeftThighOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vRightThighOut.x, vRightThighOut.y), ImVec2(vRightCalfOut.x, vRightCalfOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vLeftThighOut.x, vLeftThighOut.y), ImVec2(vLeftCalfOut.x, vLeftCalfOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vRightCalfOut.x, vRightCalfOut.y), ImVec2(vRightFootOut.x, vRightFootOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vLeftCalfOut.x, vLeftCalfOut.y), ImVec2(vLeftFootOut.x, vLeftFootOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vUpperArmRightOut.x, vUpperArmRightOut.y), ImVec2(vLowerArmRightOut.x, vLowerArmRightOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vUpperArmLeftOut.x, vUpperArmLeftOut.y), ImVec2(vLowerArmLeftOut.x, vLowerArmLeftOut.y), ESP_Color, 1.f);

						DrawLine(ImVec2(vLowerArmLeftOut.x, vLowerArmLeftOut.y), ImVec2(vHandLeftOut.x, vHandLeftOut.y), ESP_Color, 1.f);
						DrawLine(ImVec2(vLowerArmRightOut.x, vLowerArmRightOut.y), ImVec2(vHandRightOut.x, vHandRightOut.y), ESP_Color, 1.f);
...
void Render()
{
	...
	{
		InputHandler();
		ImGui::SetNextWindowSize(ImVec2(675, 430)); // 450,426
		ImGui::PushFont(DefaultFont);
		ImGui::Begin("Splitgay | flag1337", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		...
		if (CFG.tab_index == 0)
		{
			...
			if (CFG.b_Visual)
			{
				...
				ImGui::Checkbox("Information ESP", &CFG.b_EspName);
				...
				if (CFG.b_EspBox)
				{
					ImGui::Combo("Box Type", &CFG.BoxType, CFG.BoxTypes, 3);
				}
		...
		else if (CFG.tab_index == 2)
		{
			...
			ImGui::Checkbox("Airstuck (RSHIFT)", &CFG.airstuck);
			ImGui::Checkbox("Field of View Changer", &CFG.FOVChanger);

			if (CFG.FOVChanger)
			{
				ImGui::SliderFloat("FOV", &CFG.fov, 80.0f, 160.0f);
			}
		}
		ImGui::PopFont();
		ImGui::End();
...
bool IsProcessRunning(char* name)
{
...
int main()
{
	SetConsoleTitleA("SPLITGAY EXTERNAL | 18/06/2023 | MENU KEY : INSERT");
